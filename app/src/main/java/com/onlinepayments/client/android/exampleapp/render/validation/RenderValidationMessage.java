 ...
 * handles the rendering of the validation message for one payment product field
 * Copyright 2020 Global Collect Services B.V
 */
public class RenderValidationMessage implements RenderValidationMessageInterface {

    // Prefix for validation message views
    public static final String VALIDATION_MESSAGE_TAG_PREFIX = "validation_message_";

    @Override
    public void renderValidationMessage(
        String validationMessage, ViewGroup rowView, String fieldId
    ) {

        if (validationMessage == null) {
            throw new InvalidParameterException(
                "Error rendering ValidationMessage, validationMessage may not be null");
        }
        if (rowView == null) {
            throw new InvalidParameterException(
                "Error rendering ValidationMessage, rowView may not be null");
        }
        if (fieldId == null) {
            throw new InvalidParameterException(
                "Error rendering ValidationMessage, fieldId may not be null");
        }
        ...
        // Error message layout dimensions
        int TOOLTIP_TEXT_MARGIN = 9;
        layoutParams.setMargins(TOOLTIP_TEXT_MARGIN,
            TOOLTIP_TEXT_MARGIN,
            TOOLTIP_TEXT_MARGIN,
            TOOLTIP_TEXT_MARGIN
        );
        ...
        parentViewGroup.addView(validationMessageView,
    ...
    public void removeValidationMessage(ViewGroup rowView, String fieldId) {
        ...
        if (parentViewGroup != null) {
            ...
            View removeView =
                parentViewGroup.findViewWithTag(VALIDATION_MESSAGE_TAG_PREFIX + fieldId);
            if (removeView != null) {
