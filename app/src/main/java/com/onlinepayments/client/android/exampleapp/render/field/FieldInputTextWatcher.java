...
public class FieldInputTextWatcher implements TextWatcher {
    ...
    public FieldInputTextWatcher(
        ...
        if (inputDataPersister == null) {
            throw new InvalidParameterException(
                "Error creating FieldInputTextWatcher, paymentRequest may not be null");
        }
        if (paymentProductFieldId == null) {
            throw new InvalidParameterException(
                "Error creating FieldInputTextWatcher, paymentProductFieldId may not be null");
        }
        if (editText == null) {
            throw new InvalidParameterException(
                "Error creating FieldInputTextWatcher, editText may not be null");
        }
        if (addMask == null) {
            throw new InvalidParameterException(
                "Error creating FieldInputTextWatcher, addMask may not be null");
        }
    ...
    protected void applyMask(String text) {
        ...
        FormatResult applyMaskResult = inputDataPersister.getMaskedValue(paymentProductFieldId,
            text,
            oldValue,
            start,
            count,
        ...
        if (applyMaskResult != null) {
            ...
            if (!text.equals(applyMaskResult.getFormattedResult())) {
